...
def obtener_datos_seguridad(pais):
    ...
    user_sentiment = [
        i.text for i in beautiful.findAll("div", {"class": "user-sentiment-box"})
    ]
    safety_city_green = [
        i.text
        for i in beautiful.findAll("a", {"class": "score-cell-color cell-color-green"})
    ]
    safety_city_orange = [
        i.text
        for i in beautiful.findAll("a", {"class": "score-cell-color cell-color-orange"})
    ]
    warnings = [
        k.text
        for j in beautiful.findAll("div", {"class": "warning-item"})
        for k in j.findAll("h3", {"class": "anchor-heading clearfix"})
    ]
